...
def separate_script(script):
    ...
    '''
    start_sidekick_indices = [i for i, char in enumerate(script) if char == "<" and script[i:i+3] == "<s>"]
    end_sidekick_indices = [i+4 for i, char in enumerate(script) if char == "<" and script[i:i+4] == "<\s>"]
    logging.debug("start_sidekick_indices ", start_sidekick_indices)
    logging.debug("end_sidekick_indices ", end_sidekick_indices)
    assert len(start_sidekick_indices) == len(end_sidekick_indices)
    
    quotes = list(zip(start_sidekick_indices, end_sidekick_indices))
    # Sidekick has some lines
    start_position = 0
    count = 0
    lines = []
    ...
    while quotes != []:
        quote = quotes.pop(0)
        lines.append((count,0,script[start_position:quote[0]]))
        count += 1
        lines.append((count,1,script[quote[0]+3:quote[1]-4]))
        count += 1
        start_position = quote[1]
    # One more narrator line after a sidekick line before going back to you
    # Account for some punctuation
    if not start_position + 5 > len(script):
        lines.append((count,0,script[start_position:len(script)]))
